...
export default {
  ...
  vaultLine(rangeLine, prefix, variableName, variableValue) {
    ...
    execChild("echo -n '"+ variableValue + "' | " + cmd + " encrypt_string --vault-id " + passwordFile + " --stdin-name '"+ variableName +"'", { cwd: projectPath } ,(error, stdout, stderr) => {
        if (error) {
            console.log(stderr)
        } else {
            //console.log(stdout);
  ...
  unvaultLine(rangeMultipleLine, prefix, variableName, variableValue) {
    ...
    let passwordFile = atom.config.get('plugin-vault-one-line.vault_password_file_path')
    let cmd = atom.config.get('plugin-vault-one-line.path');
    execChild("echo '"+ variableValue + "' | " + cmd + " decrypt --vault-id " + passwordFile + " ", { cwd: projectPath } ,(error, stdout, stderr) => {
        if (error) {
            console.log(stderr)
        } else {
            //console.log(stdout);
